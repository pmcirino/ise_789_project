...
class TwoItem():
    ...
    def episode(self, state, action):
        ...
        I1 = x1+y1
        I2 = x2+y2
        d1 = np.random.choice(self.demand_values[0],1, list(self.demand_probabilities[0]))
        d2 = np.random.choice(self.demand_values[1],1, list(self.demand_probabilities[1]))
        cost = 0
        revenue = 0
        count =0
        new_state =0
